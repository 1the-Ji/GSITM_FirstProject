...
public class ConferenceReserveUpdateAction extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if(request.getSession().getAttribute("Employee") == null){
	    	out.println("<script type='text/javascript'>alert('로그인 후 접속해주세요.');");
    	...
    	try {
    		...
    		if(request.getParameter("resId") == null){
    			throw new RuntimeException("예약 정보 파라미터가 없습니다.");
    		}
    		int resId = Integer.parseInt(request.getParameter("resId"));
    		
    		ReservationDTO oldRes = (new ReservationDAO()).selectById(resId);
    		
    		
    		if(!(request.getSession().getAttribute("login").equals(oldRes.getEmpId()))){
    			throw new RuntimeException("예약 신청자의 ID로 로그인해주세요.");
    		}
    		
     		if( !("결재대기".equals(oldRes.getRegState()) || "승인대기".equals(oldRes.getRegState()) 
    				|| "결재완료".equals(oldRes.getRegState()) ||  "승인완료".equals(oldRes.getRegState()))){
    			throw new RuntimeException("사용완료 또는 취소된 예약은 변경할 수 없습니다.");
    		}
    		
    		String roomId = oldRes.getRoomId();
        	...
        	//예약 옵션 정보
        	List<RegOptionDTO> regOptions = (new RegOptionDAO()).select(resId);
        	String opsFixture = "";
        	String opsSnacks = "";
        	String opsSnackCounts = "";
        	for(RegOptionDTO ops : regOptions){
        		if(ops.getFixtureId() > 0){
        			opsFixture += "'" + ops.getFixtureId() + "', ";
        		}
        		if(ops.getSnackId() > 0){
        			opsSnacks += "'" + ops.getSnackId() + "', ";
        			opsSnackCounts += "'" + ops.getSnackCount() + "', ";
        		}
        	}
        	
        	//기존 예약에서 선택한 사용부서 정보 가져오기
        	List <DepOfUseRoomDTO> depOfUse = (new DepOfUseRoomDAO()).select(resId);
        	String depOfUseNames = "";
        	String depOfUseIds = "";
        	for(DepOfUseRoomDTO d : depOfUse){ //사용부서 이름 구하기
        		depOfUseIds += "'" + d.getDepId() + "', ";
        		depOfUseNames += "'" + (new DepartmentsDAO()).select(d.getDepId()).getDepName() + "', ";
        	}
        	...
        	request.setAttribute("oldRes", oldRes);					//기존 예약정보
        	request.setAttribute("room", room);						//룸 정보
        	request.setAttribute("fixtures", fixtures);				//해당 룸의 기자재 정보
        	request.setAttribute("snacks", snacks);					//해당 룸의 스낵정보
        	request.setAttribute("disableDays", disableDays); 		//선택불가일
        	request.setAttribute("opsFixture", opsFixture); 		//기존 선택한 기자재정보
        	request.setAttribute("opsSnacks", opsSnacks); 			//기존 선택한 간식정보 
        	request.setAttribute("opsSnackCounts", opsSnackCounts); //기존 선택한 간식 수량
        	request.setAttribute("depOfUseIds", depOfUseIds);		//사용부서
        	request.setAttribute("depOfUseNames", depOfUseNames);	//사용부서이름
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String[] paramCheckList 
				= {"oldResId", "periodMode", "selectedDate", "startHour", "finishHour", "startDate", "finishDate",
			...
			int oldRegId = Integer.parseInt(request.getParameter("oldResId")); //변경전 id
			//(new ReservationDAO()).update(oldRegId, reservationDTO); // --------예약 테이블 업데이트
			//기존 정보 삭제
			(new ReservationDAO()).delete(oldRegId);
			...
			//예약완료 메일 발송
			(new MailSender()).send( requestUser.getEmail(), "변경확인", resIdSeq);
			
			//결재 및 승인요청 메일발송
			(new MailSender()).send( (new EmployeesDAO()).login(approEmpId).getEmail() , approMailType, resIdSeq);
			
			out.println("<script type='text/javascript'>alert('예약 변경이 완료되었습니다.');");
			out.println("opener.parent.location.reload();window.close();</script>");
			
		} catch (Exception e) {
			out.println("<script type='text/javascript'>alert('예약 변경에 실패하였습니다.\\n" + 
					e.getMessage().replace("'", "").replace("\n", "") + "');");
