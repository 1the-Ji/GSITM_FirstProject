...
public class ConferenceManageAction extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HttpSession session = request.getSession();
			...
			List<RoomDTO> roomList = (new RoomDAO()).selectAllConRoom(intPageNo, rowsPerPage,request,buildname);
			List<String> roomNameCrList = (new RoomDAO()).selectAllCrBuildName();
			System.out.println("roomList : " + roomList);
			session.setAttribute("Room", roomList);
			session.setAttribute("roomNameCrList", roomNameCrList);
