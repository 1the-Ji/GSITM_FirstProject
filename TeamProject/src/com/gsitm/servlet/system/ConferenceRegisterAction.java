 ...
 */
@WebServlet("/conference/registeraction")
public class ConferenceRegisterAction extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("conference/registerAction : get()");
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("../view/system/conferenceRegister.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			roomDTO.setRoomId("CR" + String.format("%04d", roomIdNum));
			...
			for(int i=0; i<fixtureName.length; i++){
				fixtureDTO.setRoomId("CR" + String.format("%04d", roomIdNum));
			...
			for(int i=0; i<snackName.length; i++){
				snackDTO.setRoomId("CR" + String.format("%04d", roomIdNum));
			...
			while(files.hasMoreElements()){
				...
				roomPhotoDTO.setRoomId("CR" + String.format("%04d", roomIdNum));
				System.out.println("file_name : " + roomPhotoDTO.getPhotoName());

				(new RoomPhotoDAO()).insert(roomPhotoDTO);
			}
		}catch(Exception e){
			out.println("<script type='text/javascript'>alert('회의실 등록에 실패하였습니다.\\n" +
					e.getMessage().replace("'", "").replace("\n", "") + "');");
			out.println("window.close();</script>"); //회의실 관리자 화면으로 이동
		}
		//response.sendRedirect("../conference/manageaction");

		out.println("<script type='text/javascript'>alert('회의실을 등록하였습니다.');");
