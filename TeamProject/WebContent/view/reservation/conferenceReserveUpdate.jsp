...
since   : 2017. 5. 17.
version : 1.0
subject : 회의실 예약 변경 화면
description : 회의실 및 교육실 예약변경 입력 폼 화면
--%>
<%@page import="java.util.List"%>
<%@page import="com.gsitm.dto.RegOptionDTO"%>
<%@page import="com.gsitm.dto.FixtureDTO"%>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			if ('${param.startHour}' != '' && '${param.finishHour}' != '') {
				...
				initTimeSelecter();
				//시작시간 선택 기본값
				$("#startHour option:eq(1)").prop("selected", true);
				$("#startHour option:eq(1)").change();
			}
			...
			//예약 변경은 시간 수정 못하도록 처리 
			$("[name=periodMode]:not(:checked)").attr('disabled', 'disabled'); //체크된 라디오 이와는 disable
			//$("[name=periodMode]").css('display', 'none'); //안보이게
			
			$('#startHour option').not(':selected').attr('disabled', 'disabled');
			$('#finishHour option').not(':selected').attr('disabled', 'disabled');
			
			$('#selectedDate').attr('readonly', 'readonly');
			$('#startDate').attr('readonly', 'readonly');
			$('#finishDate').attr('readonly', 'readonly');
		...
		*/
		...
		function initDefaultValue() {
			$('#classify').val('${oldRes.useState}');
			$('#purpose').val('${oldRes.usePurpose}');
			
			$('#peoples').val(${oldRes.useCount});
			
			if('${oldRes.useNetwork}' == 'Y'){
				$('input:radio[name="networkUse"][value="Y"]').attr('checked', 'checked');
			} else {
				$('input:radio[name="networkUse"][value="N"]').attr('checked', 'checked');
			}
			
			//단기 기간 선택
			if('${oldRes.regTerm}' == 'period'){
				$('input:radio[id="radioTerm"]').attr('checked', 'checked');
				$('input:radio[id="radioTerm"]').click();
				$('#startDate').val('<fmt:formatDate value="${oldRes.startDate}" pattern="yyyy-MM-dd"/>');
				$('#finishDate').val('<fmt:formatDate value="${oldRes.finishDate}" pattern="yyyy-MM-dd"/>');
			}
			else{
				$('input:radio[id="radioDay"]').attr('checked', 'checked');
				$('input:radio[id="radioDay"]').click();
				$('#selectedDate').val('<fmt:formatDate value="${oldRes.startDate}" pattern="yyyy-MM-dd"/>');
				$('#startHour').val('<fmt:formatDate value="${oldRes.startDate}" pattern="HH:mm:00"/>');
				$('#startHour').change();
				$('#finishHour').val('<fmt:formatDate value="${oldRes.finishDate}" pattern="HH:mm:00"/>');
			}
			
			//기자재 및 간식옵션 채우기
			var fixtures = [${opsFixture}];
			var snacks = [${opsSnacks}];
			var snacksCounts = [${opsSnackCounts}];

			for(f = 0; f < fixtures.length; f++){
				$('input:checkbox[id=fixtureCheck_' + fixtures[f] + ']').attr('checked', true)
			}
			
			//간식 
			for(s = 0; s < snacks.length; s++){
				$('#snack_' + snacks[s]).val(snacksCounts[s]);
			}
			
			//사용부서 채우기
			var depOfUseNames = [${depOfUseNames}];
			var depOfUseIds = [${depOfUseIds}];
			
			for(d = 0; d < depOfUseNames.length; d++){
				setUseTeam(depOfUseIds[d], depOfUseNames[d]);
			}
		}
...
<body>
	...
	<div id="body">
		...
		<input type="hidden" name="oldResId" value="${oldRes.regInId}"/>
				...
				<td>
					<c:forEach var="fixture" items="${fixtures}" varStatus="status">
						<span class="fixtureItem">
							<input type="checkbox" id="fixtureCheck_${fixture.fixtureId}" name="fixtures" value="${fixture.fixtureId}"/>
				...
				<td>
					<c:forEach var="snack" items="${snacks}" varStatus="status">
						<span class="snackItem">
							...
							<input type="number" min="0" max="1000" value="0" id="snack_${snack.snackId}" name="snack_${snack.snackId}"/>
		...
		<div id="submitButtonContainer">
			...
			<input type="submit" class="btn-bold" value="예약 변경" style="font-size: 12pt; padding: 10px; width: 100px; margin: 10px;">
		</div>
		</form>
	</div>
</body>
