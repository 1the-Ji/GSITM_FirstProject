<%--
	...
	since   : 2017. 5. 5.
	version : 1.0
	subject : 회의실을 관리 하기위한 화면
	description : 시스템관리자가 회의실을 관리 하기 위한 폼
--%>
...
<html>
			...
			<tr>
				<td colspan=2>
					<select name="buildName" id="buildName">
						<option value="test">전체건물</option>
						<c:forEach var ="roomNameCr" items="${roomNameCrList}">
							<option value="${roomNameCr}">${roomNameCr}</option>
						</c:forEach>
					</select>
					<input id="search" type="button" onclick="selectBuildName()"value="조회" class="btn-bold"/>
				</td>
				<td style="padding-right: 0px;">
					<input type="button" onclick="moveRegisterForm()" value="+ 회의실 추가" class="btn"/>
				</td>
			</tr>
		</table>
		<table>
			<tr>
				<th>건물명</th>
				<th>회의실명</th>
				<th style="width: 28%;">회의실 정보 수정</th>
			</tr>
	...
	$(document).ready(function(){
		...
		function moveRegisterForm(){
			...
			javascript:window.open('../conference/registeraction','', 'width=910px, height=670px, left= '+(screen.availWidth/2 - 450)+', top=20');
		}
		function updateRoom(RoomId){
			//location.href="../conference/updateaction?roomId=" + RoomId;
			javascript:window.open('../conference/updateaction?roomId=' + RoomId,'', 'width=910px, height=690px, left= '+(screen.availWidth/2 - 450)+', top=20');
		}
